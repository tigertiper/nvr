...
nvrproc_close(unsigned int streamHandle)
{
	...
	if (i >= 0) {		//is recording	
		...
		if ((res = CloseRecordSeg(streamHandle, streamInfos[i])) == -1) {
			syslog(LOG_ERR,  "close record stream fail, IPC:%s, handle:0x%x, errno:%u.",pcameraID, streamHandle, ErrorFlag);
		} 
		releaseStreamInfo(&streamInfos[i]);
        syslog(LOG_INFO,  "close record stream success, IPC:%s, handle:0x%x.",pcameraID, streamHandle);
	} 
	else {			//is recording
		...
		if(i < 0)
		{
			...
			syslog(LOG_ERR,  "handle:0x%x is not exist.", streamHandle);
			return -1;
		}
		...
		if(res < 0){
			syslog(LOG_ERR,  "close download stream fail, IPC:%s, handle:0x%x, errno:%u.",pcameraID, streamHandle, ErrorFlag);
		}
		releaseDownloadInfo(streamHandle);
        syslog(LOG_INFO,  "close download stream success, IPC:%s, handle:0x%x.",pcameraID, streamHandle);
	}
	return res;
}
...
nvrproc_create(CREATEargs createargs)
{
	...
	{
		syslog(LOG_ERR,  "create record stream fail:error handle, IPC:%s, errno:%u.",createargs.camerID, ErrorFlag);
		return -1;
	}
	...
	if (i < 0) {
		...
		if (i < 0) {
			syslog(LOG_ERR,  "create record stream fail:error alloc, IPC:%s, errno:%u.",createargs.camerID, ErrorFlag);
			return -1;
		}
	...
	if (pthread_create(&streamInfos[i]->wrThread, NULL, writeThread, (void *)streamInfos[i]) != 0) {
		...
		pthread_mutex_unlock(&streamInfos[i]->Mutex_Buffer[streamInfos[i]->BFlag]);
		syslog(LOG_ERR,  "create record stream fail:create writethread fail, IPC:%s, errno:%u.",createargs.camerID, ErrorFlag);
		return -1;
	}

	syslog(LOG_INFO,  "create record stream success, IPC:%s, handle:0x%x.",createargs.camerID,  handle);
	return handle;
}
...
nvrproc_write(WRITEargs writeargs)
{
	...
	if (i < 0 || writeargs.data.data_len <= 0) {
		ErrorFlag = ERR_HANDLE;
		syslog(LOG_ERR,   "error record handle:0x%x.", writeargs.recordHandle);
		return -1;
	}
	...
	if (writeToBuf(streamInfos[i], streamInfos[i]->writeData, streamInfos[i]->writeDataLen) < 0) {
		...
		syslog(LOG_ERR,   "write data to buf fail, IPC:%s, errno:%u.", streamInfos[i]->cameraID, ErrorFlag);
		return -1;
	}
	...
	while (1) {
		...
		if (si->BUsed_Size[si->wrFlag] > 0) {
			...
			if (ret == -1) {
				...
				si->isRecording = 0;
				//pthread_rwlock_unlock(&si->RWlock_Recording);
				syslog(LOG_ERR,  "write lvm error, IPC:%s, errno:%u", si->cameraID, ErrorFlag);
			}
...
nvrproc_open(CMMNargs openargs)
{
	...
	if (handle == (unsigned int)-1) {
		syslog(LOG_ERR, "open download stream fail:error handle, IPC:%s, errno:%u.",openargs.camerID, ErrorFlag);
		return handle;
	}
	if ((i = initDownloadInfo(handle)) < 0)
	{
		syslog(LOG_ERR, "open download stream fail:max download streams, IPC:%s, errno:%u.",openargs.camerID, ErrorFlag);
		return -1;
	}
	...
	//end
	syslog(LOG_INFO,  "open download stream sucess, IPC:%s, handle:0x%x.", openargs.camerID, handle);
	return handle;
}
...
nvrproc_read(READargs readargs)
{
	...
	if(i < 0)
	{
		syslog(LOG_ERR,  "read data fail:error handle, IPC:%s, errno:%u.", pDInfo[i]->CameraID, ErrorFlag);
	}
	readLen = ReadStream(readargs.playHandle, readBuf, readargs.readSize, readargs.mode);

	//syslog(LOG_ERR,  "read %d bytes\n", readLen);
	if (readLen < 0) {
		syslog(LOG_ERR,  "read data fail, IPC:%s, errno:%u.", pDInfo[i]->CameraID, ErrorFlag);
		readLen = 0;
	}
...
nvrproc_getheader(CMMNargs gethargs)
{
	...
	{
		syslog(LOG_ERR,  "get recordseg header fail, IPC:%s, errno:%u.", gethargs.camerID, ErrorFlag);	
	...
	syslog(LOG_INFO,  "get recordseg header success, IPC:%s.", gethargs.camerID);
	return headerInfo;
}
...
{ 
	...
	if(headerInfo.nextBeginTime == ERR_RETURN)
	{
		syslog(LOG_ERR,  "search record fail, IPC:%s, errno:%u.", searchargs.camerID, ErrorFlag);		
	}
...
{    
	...
	{
		syslog(LOG_ERR, "search record fail, IPC:%s, errono:%u.", searchargs.camerID, ErrorFlag);
	}
...
{
	syslog(LOG_INFO,  "set record info, IPC:%s", setrecinfoargs.cmmnArgs.camerID);
	return 1;
}
...
nvrproc_delete(DELargs delargs)
{
	...
	if(ret < 0)
	{
		syslog(LOG_ERR, "delete record segment fail, IPC:%s, errno:%u.", delargs.camerID, ErrorFlag);
		return ret;
	}
	syslog(LOG_INFO,  "delete record segment success, IPC:%s.", delargs.camerID);
	return ret;
}
...
nvrproc_login(LOGINargs loginargs)
{
	...
	{
		syslog(LOG_ERR, "login fail:empty usrname or password, IP:%s, PORT:%u.", ClientIP, ClientPort);
		return 1;
	}
	if (strcmp(loginargs.userName, "admin")
	    || strcmp(loginargs.pwd, "password"))
	{	
		syslog(LOG_ERR, "login fail:error usrname or password, IP:%s, PORT:%u.", ClientIP, ClientPort);
		return 1;
	}
	return 0;
}
...
nvrproc_getrecsize(GETRECSIZEargs getRecSizeArgs)
{
	...
	if (headerInfo.nextBeginTime == ERR_RETURN)
	{
		syslog(LOG_ERR, "get recordseg size fail, IPC:%s, erron:%u.", getRecSizeArgs.camerID, ErrorFlag);
		return 0;
	}
	...
	syslog(LOG_INFO,  "get recordseg size success, IPC:%s.", getRecSizeArgs.camerID);
	return recordSize;
}
...
nvrproc_creatrecvol(CREATRECVOLargs creatRecVolArgs)
{
    ...
    if('\0' == *creatRecVolArgs.volumeid)
    {
        ...
        if(ret < 0)
        {
        	syslog(LOG_ERR,  "create recordvol fail, IPC:%s, errno:%u.",creatRecVolArgs.name, ErrorFlag);
        	return ret;
        }

        lv[i].length = get_free_vol_size(lv[i].lv_name);
        syslog(LOG_ERR,  "create recordvol sucess, IPC:%s, vediovol:%s.",creatRecVolArgs.name, creatRecVolArgs.volumeid);
    }
    else
    {
        ...
        syslog(LOG_INFO,  "create recordvol success, IPC:%s, vediovol:%s",creatRecVolArgs.name, creatRecVolArgs.volumeid);
    }
	return ret;
}
...
nvrproc_delrecvol(DELRECVOLargs delRecVolArgs)
{
	...
	if(ret < 0)
	{
		syslog(LOG_ERR,  "delete recordvol fail, IPC:%s, errno:%u.",delRecVolArgs.cameraID, ErrorFlag);
		return ret;
	}
	for (i = 0; i < MAX_LV_NUM; i++)
		lv[i].length = get_free_vol_size(lv[i].lv_name);
	syslog(LOG_INFO,  "delete recordvol success, IPC:%s.",delRecVolArgs.cameraID);
	return ret;
}
...
clearInactiveStreams()
{
    ...
    syslog(LOG_DEBUG,  "clear inactive streams sucess.");
}
